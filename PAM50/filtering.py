...
seq_data = pd.read_csv("../BRCA/BRCA_clinicalMatrix_filtered.csv",sep=';')
...
print(seq6.dtypes)
#print(seq_data.dtypes)

file = open("PAM50_removed.csv", "w")
file.write(";".join(seq1.columns.tolist()) + "\n")
...
def add(f, seq):
	for index, row in seq.iterrows():
		#if not row["Sample ID"] in seq_data["sampleID"]:
		#	continue
		f.write(";".join([str(row[i]) for i in types]) + "\n")

def gen():
	with open("PAM50_filtered.csv", "w") as f:
		f.write(";".join(seq1.columns.tolist()) + "\n")
		add(f,seq1)
		add(f,seq2)
		add(f,seq3)
		add(f,seq4)
